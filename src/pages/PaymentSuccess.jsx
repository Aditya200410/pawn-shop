  ...
  useEffect(() => {
    const verifyPayment = async () => {
      ...
      try {
        ...
        try {
          // You may need to create a new API endpoint to fetch order details by orderId if not present
          const statusResult = await paymentService.checkPhonePeStatus(phonePeOrderId);
          // If your backend returns merchantOrderId in the status response, extract it
          merchantOrderId = statusResult.data?.merchantOrderId || null;
          // Optionally, fetch your own order data if needed for order creation
          // orderData = await orderService.getOrderByPhonePeOrderId(phonePeOrderId);
        } catch (fetchError) {
        ...
        if (result.data?.state === 'COMPLETED') {
          ...
          setError('Payment is pending or not completed. Please check again later or contact support.');
        }
        setStatus(result);
